...
// pluginBuildStep is one command line declared by a plugin Makefile.
...
func runBuild(ctx context.Context, a *app, input commandInput) error {
	...
	if pluginsEnabled {
		if err = runPluginBuildHooks(ctx, a, env, options.Verbose); err != nil {
			return err
		}
		if err = runWasm(ctx, a, commandInput{Params: map[string]string{"out": options.OutputDir}}); err != nil {
			return err
		}
	} else {
...
func runBuildDir(ctx context.Context, a *app, input commandInput, options buildOptions, rawDir string) error {
	...
	case isOfficialPluginDir(a.root, dir):
		...
		return runOnePluginBuildHook(ctx, a, dir, env, options.Verbose)
	default:
		env := plugins.BuildEnv(a.root, a.env, false, "")
...
func runOnePluginBuildHook(ctx context.Context, a *app, pluginDir string, env []string, verbose bool) error {
	...
	if strings.EqualFold(strings.TrimSpace(manifest.Type), "dynamic") {
		return runWasm(ctx, a, commandInput{Params: map[string]string{"plugin_dir": pluginDir}})
	}
	steps, err := resolvePluginMakefileBuildSteps(a.root, pluginDir)
	if err != nil {
		return err
	}
	if len(steps) == 0 {
		fmt.Fprintf(a.stdout, "No plugin build hook found: %s\n", toolutil.RelativePath(a.root, pluginDir))
		return nil
	}
	fmt.Fprintf(a.stdout, "Building plugin: %s\n", toolutil.RelativePath(a.root, pluginDir))
	return runPluginBuildSteps(ctx, a, pluginDir, env, verbose, steps)
}
...
// runPluginBuildHooks runs plugin-owned build hooks before Go compilation so
// plugins can generate embeddable files without storing build artifacts in Git.
func runPluginBuildHooks(ctx context.Context, a *app, env []string, verbose bool) error {
	plugins, err := discoverPluginBuildHookRoots(a.root)
	if err != nil {
		return err
	}
	if len(plugins) == 0 {
		fmt.Fprintln(a.stdout, "No plugin build hooks found")
		return nil
	}
	fmt.Fprintf(a.stdout, "Building %d plugin build hook(s)...\n", len(plugins))
	for _, pluginDir := range plugins {
		fmt.Fprintf(a.stdout, "Building plugin: %s\n", toolutil.RelativePath(a.root, pluginDir))
		steps, stepsErr := resolvePluginMakefileBuildSteps(a.root, pluginDir)
		if stepsErr != nil {
			return stepsErr
		}
		if err = runPluginBuildSteps(ctx, a, pluginDir, env, verbose, steps); err != nil {
			return err
		}
	}
	return nil
...
// discoverPluginBuildHookRoots returns plugin roots with a plugin-owned build
// hook. Plugins without generated content can omit the hook.
func discoverPluginBuildHookRoots(root string) ([]string, error) {
	matches, err := filepath.Glob(filepath.Join(root, "apps", "lina-plugins", "*", "Makefile"))
	if err != nil {
		return nil, fmt.Errorf("scan plugin build hooks: %w", err)
	}
	plugins := make([]string, 0, len(matches))
	for _, makefile := range matches {
		pluginDir := filepath.Dir(makefile)
		steps, stepsErr := resolvePluginMakefileBuildSteps(root, pluginDir)
		if stepsErr != nil {
			return nil, stepsErr
		}
		if len(steps) > 0 {
...
// resolvePluginMakefileBuildSteps reads PLUGIN_BUILD_STEP_N variables from the
// plugin Makefile so the plugin owns its build command while linactl keeps the
// repository-level build dispatcher cross-platform.
func resolvePluginMakefileBuildSteps(root string, pluginDir string) ([]pluginBuildStep, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("read plugin build Makefile %s: %w", makefilePath, err)
	}
	...
	for _, line := range strings.Split(string(data), "\n") {
		...
		key, value, ok := parseMakefileVariable(trimmed)
		if ok {
			variables[key] = value
		}
	}
	variables["PLUGIN_ROOT"] = pluginDir
	variables["REPO_ROOT"] = root

	steps := []pluginBuildStep{}
	for index := 1; ; index++ {
		raw, ok := variables[fmt.Sprintf("PLUGIN_BUILD_STEP_%d", index)]
		if !ok {
			break
		}
		raw = strings.TrimSpace(expandMakefileVariables(raw, variables))
		if raw == "" {
			continue
		}
		fields, splitErr := splitBuildCommandLine(raw)
		if splitErr != nil {
			return nil, fmt.Errorf("parse plugin build step %d in %s: %w", index, makefilePath, splitErr)
		}
		if len(fields) == 0 {
			continue
		}
...
// parseMakefileVariable parses the simple variable assignments used by plugin
// Makefiles for linactl-visible build steps.
func parseMakefileVariable(line string) (string, string, bool) {
	for _, operator := range []string{":=", "?=", "="} {
		key, value, ok := strings.Cut(line, operator)
		if !ok {
			continue
		}
...
// expandMakefileVariables expands the small $(NAME) and ${NAME} subset needed
// by plugin build step declarations.
func expandMakefileVariables(value string, variables map[string]string) string {
	expanded := value
	for i := 0; i < 8; i++ {
		next := expanded
...
func hasPackageBuildScript(manifestPath string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("read plugin build hook manifest %s: %w", manifestPath, err)
	}
	var manifest struct {
		Scripts map[string]string `json:"scripts"`
	}
	if err = json.Unmarshal(data, &manifest); err != nil {
		return false, fmt.Errorf("parse plugin build hook manifest %s: %w", manifestPath, err)
	}
