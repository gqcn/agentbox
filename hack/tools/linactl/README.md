...
Source plugins keep their own build commands in the plugin root `Makefile` through `PLUGIN_BUILD_STEP_1`, `PLUGIN_BUILD_STEP_2`, and so on. `linactl build` reads those step declarations and executes them directly, so root builds do not require GNU Make to run plugin hooks.
