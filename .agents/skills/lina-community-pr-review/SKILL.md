...
description: >-
  审查 LinaPro 社区 GitHub Pull Request，并按项目规范发表评论。用户要求审查 LinaPro PR、社区 PR、pull request、bot 审批、bot-approved 标签、GitHub PR 治理，或提到 lina-community-pr-review 时必须使用本技能。默认审查 https://github.com/linaproai/linapro；用户指定 PR 编号时只审查指定 PR，否则扫描全部开放 PR；跳过 bot-approved PR；无新提交时不重复审查；发现问题时评论指出问题和修改建议；无法处理时评论并 @ 相关历史维护成员；完全符合规范时添加 bot-approved 标签。
...
9. 如果`PR`存在问题，创建或更新一条粘性审查评论，列出问题和修改建议。
...
审查姿态与`lina-review`一致：优先检查正确性、项目规则违反、安全或权限缺口、性能回归、测试缺失和治理违规。发现问题时尽量提供文件路径和行号。补丁无法提供行号时，引用文件以及最近的函数、章节或变更块。
...
问题：
- **严重** `<file>:<line>`：<问题说明>。依据：`<rule-file>`。建议：<修改建议>。
- **警告** `<file>:<line>`：<问题说明>。依据：`<rule-file>`。建议：<修改建议>。

本次未添加`bot-approved`标签。
```
...
Automated review found issues that need changes.

Review basis:
- `<rule-file>`

Findings:
- **Critical** `<file>:<line>`: <issue>. Source: `<rule-file>`. Suggested fix: <suggestion>.
- **Warning** `<file>:<line>`: <issue>. Source: `<rule-file>`. Suggested fix: <suggestion>.

The `bot-approved` label was not added.
```
...
自动审查无法完成，需要人工 review。

阻断原因：
- <原因>

建议请以下成员协助 review：@alice @bob
...
Automated review could not be completed and needs human review.
...
Selection basis:
- @alice previously modified: `<file>`
- @bob previously modified: `<file>`

Suggested focus:
- <item that needs human judgment>
...
- 中文：`未能从相关文件历史中确认可@的项目成员。`
- 英文：`No project member could be confirmed from the affected file history.`
