...
必须使用以下模板，除替换 `<to>`、`<from>` 和正文外，不新增其他二级章节：
...
1. 文件以 `# LinaPro Release <to>` 开头。
2. `Source range` 与规范化范围一致。
3. 只包含固定模板章节。
4. 中间分割线存在。
5. 中文标题和来源范围存在。
6. 英文和中文事实覆盖一致。
7. 英文和中文表达地道、清晰、流利、易懂，没有生硬直译、罕见词或拗口表达。
8. 没有把软件语境中的 `seam`、`fixture` 等词机械翻译成“接缝”“夹具”等不自然说法。
9. 每个章节要么有证据支持的内容，要么写明未识别到相关变更。
