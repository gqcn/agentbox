...
当`Issue`是功能需求类请求时，技能 SHALL 根据项目定位、项目规范和源码实现评估可行性。评估可行时，技能 SHALL 添加`feature`标签，发布评估结果评论，并保持`Issue`开放等待实现。
...
当`Issue`是`Bug`类请求时，技能 SHALL 根据项目规范和源码实现评估可能原因。评估可行且需要修复时，技能 SHALL 添加`bug`标签，发布评估结果评论，并保持`Issue`开放等待修复。
...
技能 SHALL 在处理结束后向用户报告审查仓库、扫描数量、跳过数量、分类处理数量、关闭数量和阻断原因。
...
- **AND** 技能报告已回答并关闭的疑问类`Issue`、已添加`feature`标签的`Issue`、已添加`bug`标签的`Issue`和已关闭的无效`Issue`
